<x-app-layout>
                    ...
                    @if (count($products) > 0)
                        @foreach ($products as $product )
                            <div class="md:flex md:items-center mb-4 justify-around">
                                <div class="md:w-3/12">@if ($product->imageThird->filename !== null)
                                ...
                                <form method="post" action="{{ route('user.cart.destroy', ['cart' => $product->id]) }}">
                                    @csrf
                      ...
                      小計: {{ number_format($totalPrice)}}<span class="text-sm text-gray-700">円(税込)</span>
                        ...
                        <div>
                            <button onclick="@if($user->address)location.href='{{ route('user.cart.payment') }}' @else userAddress() @endif" class="w-full m-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">購入する</button>
                        </div>
                    @else
                        カートに商品が入っていません
                    @endif
    ...
    <script>
        function userAddress() {
            let address = confirm("発送先住所が登録されていません、登録をお願いします");
            if (address == true){
